...
$query = "
    ...
    WHERE user_id = ? AND current_status = 'Declined'
               ...
               <div class="work-progres">
                            <div class="chit-chat-heading" style="font-size:30px;text-align:center;color:#000;margin-bottom:10px;display:flex;align-items:center;justify-content:space-around;">
                               ...
                               <div class="btn-effcts panel-widget" style="margin:0px;padding:0px;padding-left: 5px;padding-right: 5px;background: #d9d9d9;width:auto;">
                                      <a href="approved-forms.php" class="hvr-icon-float-away hvr-trim" style="text-decoration:none !important;">Approved Forms</a>
                                  </div>
                            </div>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($service = $service_result->fetch_assoc()) {
                      ...
                      $actiontext = ''; // Default value to avoid undefined variable warning
$created_at = $row['created_at']; // Use per-row created_at

$getting_action = $conn->prepare("SELECT action_text FROM form_submission_responses WHERE user_id = ? AND service_id = ? AND created_at = ?");
